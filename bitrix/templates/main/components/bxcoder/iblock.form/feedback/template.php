...
<form name="iblock_add" action="<?=POST_FORM_ACTION_URI?><?if($arParams["SCROLL_TO_FORM"]):?>#form<?=$arParams["IBLOCK_ID"]?><?endif?>" method="post" enctype="multipart/form-data">
	...
	<?if ($arParams["MAX_FILE_SIZE"] > 0):?><input type="hidden" name="MAX_FILE_SIZE" value="<?=$arParams["MAX_FILE_SIZE"]?>" /><?endif?>
		<?if (is_array($arResult["PROPERTY_LIST"]) && !empty($arResult["PROPERTY_LIST"])):?>
			<?foreach ($arResult["PROPERTY_LIST"] as $propertyID):?>
      			<div class="form-group">
					<?=printProperty($propertyID, $arResult, $arParams)?>
				</div>
			<?endforeach;?>
			<?if($arParams["USE_CAPTCHA"] == "Y" && $arParams["ID"] <= 0):?>
				<div class="form-group">
					...
					<div class="captcha-group">
						...
						<input class="form-control" type="text" name="captcha_word" maxlength="50" value="" placeholder="<?=GetMessage("IBLOCK_FORM_CAPTCHA_PROMPT")?>">
					</div>
				</div>
			<?endif?>
		</tbody>
		...
		<div class="text-right">
					<input type="submit" class="btn btn-default" name="iblock_submit" value="<?=GetMessage("IBLOCK_FORM_SUBMIT")?>" />
					<?if (strlen($arParams["LIST_URL"]) > 0):?>
						<input class="question_btn" type="submit" name="iblock_apply" value="<?=GetMessage("IBLOCK_FORM_APPLY")?>" />
						<input
							type="button"
							name="iblock_cancel"
