...
{
  function printProperty($propertyID, $arResult, $arParams, $placeholder)
  {
		...
		switch ($INPUT_TYPE):
			...
			case "TAGS":
							...
							case "T":
								...
								?>
						<textarea cols="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]?>" rows="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["ROW_COUNT"]?>" name="PROPERTY[<?=$propertyID?>][<?=$i?>]" placeholder="<?printPropertyName($propertyID, $arResult, $arParams)?>"><?=$value?></textarea>
								<?
								}
							break;
							...
							case "N":
								...
								?>
								<input type="text" name="PROPERTY[<?=$propertyID?>][<?=$i?>]" size="25" value="<?=$value?>" placeholder="<?printPropertyName($propertyID, $arResult, $arParams)?>" /><?
							...
							case "F":
								for ($i = 0; $i<$inputNum; $i++)
								{
									$value = intval($propertyID) > 0 ? $arResult["ELEMENT_PROPERTIES"][$propertyID][$i]["VALUE"] : $arResult["ELEMENT"][$propertyID];
									?>
						<input type="hidden" name="PROPERTY[<?=$propertyID?>][<?=$arResult["ELEMENT_PROPERTIES"][$propertyID][$i]["VALUE_ID"] ? $arResult["ELEMENT_PROPERTIES"][$propertyID][$i]["VALUE_ID"] : $i?>]" value="<?=$value?>" />        <label><?printPropertyName($propertyID, $arResult, $arParams)?></label>
...
<form name="iblock_add" action="<?=POST_FORM_ACTION_URI?><?if($arParams["SCROLL_TO_FORM"]):?>#form<?=$arParams["IBLOCK_ID"]?><?endif?>" method="post" enctype="multipart/form-data">
	...
	<?if ($arParams["MAX_FILE_SIZE"] > 0):?><input type="hidden" name="MAX_FILE_SIZE" value="<?=$arParams["MAX_FILE_SIZE"]?>" /><?endif?>
        ...
        <div class="field">    
				  <div class="input"><?=printProperty($propertyID, $arResult, $arParams)?></div>
				</div>
			<?endforeach;?>
