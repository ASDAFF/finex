...
$arParams["SCROLL_TO_FORM"] = $arParams["SCROLL_TO_FORM"] === "Y";
    ...
    if (check_bitrix_sessid() && (!empty($_REQUEST["iblock_submit"]) || !empty($_REQUEST["iblock_apply"]))) {
        ...
        if ($arParams["USE_CAPTCHA"] == "Y" && $arParams["ID"] <= 0) {
            if (!$APPLICATION->CaptchaCheckCode($_REQUEST["captcha_word"], $_REQUEST["captcha_sid"])) {
                ...
                if (!empty($recaptcha) or !empty($_SESSION['g-recaptcha-response'])) {
                    if (!empty($_SESSION['g-recaptcha-response'])) $recaptcha = $_SESSION['g-recaptcha-response'];
                    else {
                        $_SESSION['g-recaptcha-response'] = $recaptcha;
                        $google_url = "https://www.google.com/recaptcha/api/siteverify";
                        $secret = '6LeJxksUAAAAAO3wfvh1t6-Eddq5W50mmY9MyfM4';
        ...
        if (empty($arResult["ERRORS"])) {
            ...
            if ($arParams["ID"] > 0) {
                ...
                if (strlen($arUpdateValues["NAME"]) <= 0) {
                    $iblockFields = CIblock::GetFields($arParams["IBLOCK_ID"]);
                    $arUpdateValues["NAME"] = str_replace("#date#", date("d.m.Y"), $iblockFields["NAME"]["DEFAULT_VALUE"]);
                }
                ...
                while ($arProperty = $properties->GetNext()) {
                    if (!in_array($arProperty["CODE"], $arParams["PROPERTY_CODES"])) {
                        if ($arProperty["DEFAULT_VALUE"]) {
                            $arUpdateValues["PROPERTY_VALUES"][$arProperty["ID"]] = $arProperty["DEFAULT_VALUE"];
                        } elseif ($arProperty["PROPERTY_TYPE"] == "L") {
                            $property_enums = CIBlockPropertyEnum::GetList(Array("ID" => "ASC"), Array("IBLOCK_ID" => $arParams["IBLOCK_ID"], "CODE" => $arProperty["CODE"]));
                            while ($enum_fields = $property_enums->GetNext()) {
                                if ($enum_fields["DEF"] == "Y")
                                    $arUpdateValues["PROPERTY_VALUES"][$arProperty["ID"]] = $enum_fields["ID"];
                            }
                        }
                    }
                }
                ...
                if ($arParams["DEFAULT_VALUES"]) {
                    foreach ($arParams["DEFAULT_VALUES"] as $code => $value) {
                        $arUpdateValues["PROPERTY_VALUES"][$code] = $value;
                    }
                }
                ...
                if (!$arResult["ERRORS"]) {
                    ...
                    if ($arParams["EVENT_TYPE"]) {
                        $MESSAGE = '';
                        foreach ($arResult["PROPERTY_LIST_FULL"] as $index => $prop) {
                            if (in_array($index, $arParams["PROPERTY_CODES"])) {
                                if ($prop["CODE"]) {
                                    $code = $prop["CODE"];
                                    $value2 = $arUpdatePropertyValues[$index];
                                    $value = $arUpdatePropertyValues[$index];
                                    if (is_array($arUpdatePropertyValues[$index])) {
                                        $value = '';
                                        foreach ($arUpdatePropertyValues[$index] as $propValue)
                                            $value .= $propValue . "\n";
                                    }
                                } else {
                                    $code = $index;
                                    $value = $arUpdateValues[$index];
                                }

                                if ($prop["PROPERTY_TYPE"] == "L") {
                                    $property_enums = CIBlockPropertyEnum::GetList(Array(), Array("IBLOCK_ID" => $prop["IBLOCK_ID"], "CODE" => $prop["CODE"], "ID" => $value2));
                                    while ($enum_fields = $property_enums->GetNext()) {
                                ...
                                if ($prop["PROPERTY_TYPE"] == "F" && $value) {
                                    $value = '';
                                    $db_props = CIblockElement::GetProperty($arParams["IBLOCK_ID"], $arParams["ID"], array(), array("CODE" => $prop["CODE"]));
                                    while ($ar_props = $db_props->Fetch()) {
                                        if ($ar_props["VALUE"] > 0) {
                                            $file = CFile::GetPath($ar_props["VALUE"]);
                                            if ($file) {
                                                $value .= 'http://' . SITE_SERVER_NAME . $file . '<br>';
                                            }
                                        }
                                    }
                                    $MESSAGE .= '<br>' . $value;
                        ...
                        $arEventFields['MESSAGE2'] = $MESSAGE;
                        $arEventFields['SITE_URL'] = substr($_SERVER[REQUEST_URI],-3,2);
                       
                        if($arEventFields['SITE_URL']=='_e')$arEventFields['SITE_URL']='en';
                        if($arEventFields['SITE_URL']!='en')$arEventFields['SITE_URL']='ru';
                        if ($arParams["DEFAULT_VALUES"]) {
                            foreach ($arParams["DEFAULT_VALUES"] as $code => $value) {
                                $arEventFields[$code] = $value;
                            }
                        ...
                        CEvent::Send($arParams["EVENT_TYPE"], SITE_ID, $arEventFields);
                    }
                }
            }
        }
